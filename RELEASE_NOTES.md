...
RC.2 fixes the root installer variable collision found during RC.1 installation validation. The application runtime is otherwise unchanged from the live-tested RC.1 commit.
...
RC.2 修复了 RC.1 本机安装验证时发现的安装器版本变量覆盖问题；应用运行时与已完成真实模型测试的 RC.1 保持一致。
